...
AddEventHandler('fami-sell-vehicles:buyVehicle', function (index)
    ...
    if index ~= '0' then index = tonumber(index) else index = 0 end
    if index == nil then error("Invalid index") end
    ...
    if result[1] then
        ...
        if xPlayer.getMoney() >= price then
            ...
            -- Insere na garagem correta
            MySQL.insert.await('INSERT INTO owned_vehicles (owner, plate, vehicle, `stored`, type, parking) VALUES (@owner, @plate, @vehicle, 1, "car", @parking)', {
                ['@owner'] = xPlayer.getIdentifier(),
                ['@plate'] = vehicleProps.plate,
                ['@vehicle'] = json.encode(vehicleProps),
                ['@parking'] = "Pillbox Hill"  -- garagem correta
            })
